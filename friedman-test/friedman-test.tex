...
For objects a general set $\mathcal{X}$, we can define a mapping $\phi$ into a
...
With an RBF kernel (width 1), it appears that $C$ has a small effect.
\begin{figure}
  \begin{center}
    \resizebox{14.0cm}{!}{
...
\item $\phi({\bf x}) = $ the number of length $k$ contiguous
  subsequences ($k$-mers) in ${\bf x}$.
\item $\mathcal{H} = \mathbb{N}^{|\mathcal{A}|^k}$.
\item $K_k({\bf x}, {\bf x}') = \langle \phi_k({\bf x}), \phi_k({\bf x}') \rangle$.
\end{itemize}
...
\item $K_d({\bf x},{\bf x}') = ({\bf x}^T {\bf x}'  + c)^d$ is $\mathcal{O}(n)$.
\end{itemize}
