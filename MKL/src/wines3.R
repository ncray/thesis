...
winePower <- function(n, mkl_norm = 2, C = .01){
  ...
  rej <- with(dat, reject(compute(trainMKL), parametric = TRUE, verbose = FALSE)(desc = desc, subregion = subregion, price = price, ratings = ratings, year = year, title = title, l = l, desc.v = 3, title.v = 3, mkl_norm = mkl_norm, C = C))
  wts <- getMKLWeights()
  names(wts) <- c("desc.K3", "subr.C", "price.L", "ratings.L", "year.L", "title.K3")
  c("reject" = rej, "n" = n, wts, group = "Desc./SubR./Price/Ratings/Year/Title")
}

system.time(res1 <- ldply(rep(c(seq(50, 350, 50)), each = 50), winePower, .parallel = TRUE))

winePower <- function(n, mkl_norm = 2, C = .01){
  dat <- getWineData(n, dat3)
  rej <- with(dat, reject(compute(trainMKL), parametric = TRUE, verbose = FALSE)(desc = desc, subregion = subregion, price = price, ratings = ratings, year = year, title = NULL, l = l, desc.v = 3, title.v = NULL, mkl_norm = mkl_norm, C = C))
  wts <- getMKLWeights()
  names(wts) <- c("desc.K3", "subr.C", "price.L", "ratings.L", "year.L")
  c("reject" = rej, "n" = n, wts, group = "Desc./SubR./Price/Ratings/Year")
}

system.time(res2 <- ldply(rep(c(seq(50, 350, 50)), each = 50), winePower, .parallel = TRUE))

winePower <- function(n, mkl_norm = 2, C = 3){
  print(n)
  dat <- getWineData(n, dat3)
  rej <- with(dat, reject(compute(trainMKL), parametric = TRUE, verbose = FALSE)(desc = desc, subregion = NULL, price = price, ratings = ratings, year = year, title = NULL, l = l, desc.v = 3, title.v = NULL, mkl_norm = mkl_norm, C = C))
  wts <- getMKLWeights()
  names(wts) <- c("desc.K3", "price.L", "ratings.L", "year.L")
  c("reject" = rej, "n" = n, wts, group = "Desc./Price/Ratings/Year", "C" = C)
}

#system.time(res3.2 <- ldply(rep(c(.5, 1, 5, 10, 100), each = 5), function(C) winePower(500, 2, C), .parallel = TRUE))
#system.time(res3.2 <- ldply(rep(as.numeric(4:8), each = 50), function(C) winePower(350, 2, C), .parallel = TRUE))
##     n C   V1
## 1 350 1 0.08
## 2 350 2 0.10
## 3 350 3 0.34
...
## 4 350 7 0.20
## 5 350 8 0.12
#ddply(res3.2, .(n, C), function(df) mean(as.logical(df$reject.t)))

system.time(res3 <- ldply(rep(c(seq(50, 350, 50)), each = 50), winePower, .parallel = TRUE))

res <- rbind(res1[, c("reject.t", "n", "group")], res2[, c("reject.t", "n", "group")], res3[, c("reject.t", "n", "group")])
res$reject.t <- as.logical(res$reject.t)
...
winePower <- function(n, mkl_norm = 2, C = .01){
  dat <- getWineData(n, dat3)
  rej <- with(dat, reject(compute(trainMKL), parametric = TRUE, verbose = FALSE)(desc = NULL, subregion = NULL, price = price, ratings = ratings, year = NULL, title = NULL, l = l, desc.v = NULL, title.v = NULL, mkl_norm = mkl_norm, C = C))
  wts <- getMKLWeights()
  names(wts) <- c("price.L", "ratings.L")
  c("reject" = rej, "n" = n, wts)
}

system.time(res <- ldply(rep(c(seq(50, 250, 50)), each = 20), winePower, .parallel = TRUE))
qplot(x = n, y = V1, data = ddply(res, .(n), function(df) mean(df$reject.t)), geom = "line")
res.m <- melt(res[, -1], id.vars = "n")
